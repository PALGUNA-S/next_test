...
import { Card, CardContent, CardMedia, Grid, Typography, Box, Button, Stack, Skeleton, Container, } from "@mui/material";
...
const BlogList = () => {
  ...
  const filterDataByCategory = (category) => {
    if (category === "all") {
      ...
      return blogData.filter((item) => item.category === category);
    }
  };
  ...
  return (
    <section style={{ backgroundColor: '#eff2fc' }} >
      <Container>
        <Typography pb={1} variant="h1" sx={{ fontWeight: "600", fontSize: '2.25rem' }} > Blogs</Typography>
        <Stack direction={"row"} spacing={1} pb={2}>
          ...
          <Button
            ...
            sx={{
              borderRadius: '0', textTransform: 'none', fontSize: "1rem", color: '#142d82', borderBottom: selectedCategory === "founder" ? "2px solid #142d82" : "none",
                ...
                <Link href={`/blogs/${blogData.link}`}>
                  {loading ? (
                    ...
                    <>
                      <Card
                        sx={{
                          border: "none", boxShadow: "none", borderRadius: 0, height: "100%", transition: "background-color 0.8s, color 0.9s",
                          "&:hover": { backgroundColor: "#142d82", color: "white", },
                        }}
                      >
                        <CardMedia component="img" alt={blogData.alt} image={blogData.img} sx={{ overflow: "hidden", transition: "transform 1.2s", "&:hover": { transform: "scale(1.1)", }, }} />

                        <CardContent sx={{ paddingBottom: "1px" }}>
                          <Typography variant="h6" fontWeight={500}> {blogData.text} </Typography>
                        </CardContent>
                      </Card>
                    </>
                  )}
